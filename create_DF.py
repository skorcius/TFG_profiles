...
from lib.apriori import apriori, print_report
...
def create_data_frame_ext(alumns):
    ...
    #do_ext_assoc_rules(df_alum_assig)
    do_assoc_rules(df_alum_assig)
    return
...
def do_assoc_rules(data_o):
    data = (pd.DataFrame.copy(data_o))

    print data

    rules, item_set = apriori(data, min_support=20, min_confidence=20)
    print_report(rules, item_set)
    return
